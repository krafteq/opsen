...
export function createAzureTestEnvironment(opts?: { testId?: string; location?: string }): AzureTestEnvironment {
  ...
  execFileSync('az', ['group', 'create', '--name', rgName, '--location', location, '--output', 'none'], {
    stdio: 'inherit',
    timeout: 60_000,
  })
  ...
  const envIdOutput = execFileSync(
    'az',
    ['containerapp', 'env', 'show', '--name', envName, '--resource-group', rgName, '--query', 'id', '-o', 'tsv'],
    {
      stdio: 'pipe',
      timeout: 30_000,
    },
...
export function destroyAzureTestEnvironment(resourceGroupName: string): void {
  try {
    ...
    execFileSync('az', ['group', 'delete', '--name', resourceGroupName, '--yes', '--no-wait'], {
      stdio: 'inherit',
      timeout: 30_000,
    })
  } catch (err) {
